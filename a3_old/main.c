...
int main(int argc, char **argv)
{
	...
	DEBUGR0("Ping-Pong: %d processes, 1 - %d bytes", nproc, MAX_MSG_LEN);
	
    }



    for (uint32_t len=1; len <= MAX_MSG_LEN; len <<= 1)
    {
	...
	if (!myrank) {
	    ...
	    DEBUG("Got all process results for %u bytes: %f ns/byte", len, norm_runtime);
	    fprintf(fd, "%u,%f\n", len, norm_runtime);
	}
